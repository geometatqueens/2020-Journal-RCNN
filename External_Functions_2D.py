...
class Grid():
    ...
    def DC_Matrix(self):
        #Matrix = np.copy(self.DataConditioning[self.delta_x:self.SGsizex+self.delta_x,self.delta_y:self.delta_y+self.SGsizey,0])
        ...
        with tf.device('/gpu:0'):
            ...
            for ind_g in range(self.NumGrid):
                ...
                for ind0 in range(Path[str(ig)].shape[0]):   
                    ...
                    if DC_Position_a[xi][xj] == 1: # and np.count_nonzero(self.Values[xi - self.delta_x : xi + self.delta_x +1, xj - self.delta_y : xj + self.delta_y + 1, 0]) > self.MinDC:   
                        dd = 2**ig
                        ...
                        IP_final = argmax_y_conv_a.eval(feed_dict={x_image_a: SimInPut_a, InTrain:False})[0] + 1 # ACA SE ESTRA TRABAJANDO CON HOTSPOTCENTERVALUE0N1
                        ...
                        self.Values[cx:dx:dd,cy:dy:dd,1] += np.multiply(IP_final, TempDC) # Resimulate non conditional points #[delta_x-d_Sim_x:delta_x+d_Sim_x+1, delta_y-d_Sim_y:delta_y+d_Sim_y+1]
                        DC_Position_a[cx:dx:dd,cy:dy:dd] = np.multiply(DC_Position_a[cx:dx:dd,cy:dy:dd],(1-TempDC)) # convert ins                    
                        
                        # Assigning simulated data in the whole pattern
                        #self.Values[ax:bx:dd,ay:by:dd,1] += np.multiply(SimCenterValue, DC_Position_a[ax:bx:dd,ay:by:dd]) # Resimulate non conditional points
                        #DC_Position_a[ax:bx:dd,ay:by:dd] = 0                    
            ...
            for ind_g in range(self.NumGrid):
                ...
                for ind0 in range(Path[str(ig)].shape[0]):   
                    ...
                    if DC_Position_b[xi][xj] == 1: # and np.count_nonzero(self.Values[xi - self.delta_x : xi + self.delta_x +1, xj - self.delta_y : xj + self.delta_y + 1, 0]) > self.MinDC:   
                        dd = 2**ig
                        ax, bx = xi-self.delta_x*2**ig, xi+self.delta_x*2**ig+1
                        ay, by = xj-self.delta_y*2**ig, xj+self.delta_y*2**ig+1
                        ...
                        IP_final = argmax_y_conv_b.eval(feed_dict={x_image_b: SimInPut_b, InTrain:False})[0] + 1 # ACA SE ESTRA TRABAJANDO CON HOTSPOTCENTERVALUE0N1
            ...
            for ind_g in range(self.NumGrid):
                ...
                for ind0 in range(Path[str(ig)].shape[0]):   
                    ...
                    if DC_Position_c[xi][xj] == 1: # and np.count_nonzero(self.Values[xi - self.delta_x : xi + self.delta_x +1, xj - self.delta_y : xj + self.delta_y + 1, 0]) > self.MinDC:   
                        dd = 2**ig
                        ax, bx = xi-self.delta_x*2**ig, xi+self.delta_x*2**ig+1
                        ay, by = xj-self.delta_y*2**ig, xj+self.delta_y*2**ig+1
                        ...
                        IP_final = argmax_y_conv_c.eval(feed_dict={x_image_c: SimInPut_c, InTrain:False})[0] + 1 # ACA SE ESTRA TRABAJANDO CON HOTSPOTCENTERVALUE0N1
            ...
            for ind_g in range(self.NumGrid):
                ...
                for ind0 in range(Path[str(ig)].shape[0]):   
                    ...
                    if DC_Position_d[xi][xj] == 1: # and np.count_nonzero(self.Values[xi - self.delta_x : xi + self.delta_x +1, xj - self.delta_y : xj + self.delta_y + 1, 0]) > self.MinDC:   
                        dd = 2**ig
                        ax, bx = xi-self.delta_x*2**ig, xi+self.delta_x*2**ig+1
                        ay, by = xj-self.delta_y*2**ig, xj+self.delta_y*2**ig+1
                        ...
                        IP_final = argmax_y_conv_d.eval(feed_dict={x_image_d: SimInPut_d, InTrain:False})[0] + 1 # ACA SE ESTRA TRABAJANDO CON HOTSPOTCENTERVALUE0N1
...
def CreateGraph_4ConvNets_4HL_NFeaConv_wdnhxwdnh_BN(HyperPar, LocModel='Models/Default/FeatMaps'):
    with tf.device('/gpu:0'):
        ...
        with tf.control_dependencies(update_ops):
            #train_step = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize((cross_entropy_a + cross_entropy_b + cross_entropy_c + cross_entropy_d))
